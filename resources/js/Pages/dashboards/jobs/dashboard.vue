...
<template>
    ...
    <div class="row row-cols-xxl-5 row-cols-xl-3 row-cols-lg-3 row-cols-sm-2 row-cols-1">
        <div class="col" v-for="(idx) in JobsData.JobsCards" :key="idx.id">
            <SpkReusebleJobs :jobsCard="true" cardClass="card" :list='idx' :imageIcon="false" :NoCountUp="true" />
        </div>
    </div>
    <!-- End:: row-1 -->
                ...
                <div class="card-body p-0">
                    <ul class="list-group list-group-flush">
                        <simplebar id="recent-jobs">
                            <li :class="`list-group-item border-top-0 border-start-0 border-end-0 ${idx.id===6?'border-bottom-0':''}`"
